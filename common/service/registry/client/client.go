...
import (
	...
	"gitee.com/we7coreteam/w7-registry-cache/common/service/registry/client/auth"
	"gitee.com/we7coreteam/w7-registry-cache/common/service/registry/client/auth/bearer"
	"gitee.com/we7coreteam/w7-registry-cache/common/service/registry/client/interceptor"
	...
	"gitee.com/we7coreteam/w7-registry-cache/common/service/registry/util"
...
type Client interface {
	...
	BlobExist(repository, digest string) (exist bool, size int64, sdigest string, err error)
...
func getHttpClientOpts(reg *types.Registry) func(cli *http.Client) {
	var httpClientOpts func(cli *http.Client)
	if reg.ProxyUrl != "" {
		httpClientOpts = func(cli *http.Client) {
			...
			proxyURL, err := url.Parse(reg.ProxyUrl)
			if err != nil {
				return
			}
...
func NewClient(url, username, password string, insecure bool, interceptors ...interceptor.Interceptor) Client {
	...
	return NewClientWithAuthorizer(url, authorizer, insecure, nil, interceptors...)
}

func NewClientWithRegistry(reg *types.Registry) Client {
	authType, username, password, token := "", "", "", ""
	if reg.Credential != nil {
		authType = reg.Credential.Type
		username = reg.Credential.AccessKey
		password = reg.Credential.AccessSecret
		token = reg.Credential.Token
	}

	httpClientOpts := getHttpClientOpts(reg)
	if authType == types.CredentialTypeOAuth && token != "" {
		authorizer := bearer.NewLocalAuthorizer(token)
		return NewClientWithAuthorizer(reg.URL, authorizer, reg.Insecure, httpClientOpts)
	} else {
		authorizer := auth.NewAuthorizer(username, password, reg.Insecure)
		return NewClientWithAuthorizer(reg.URL, authorizer, reg.Insecure, httpClientOpts)
	}
}

// NewClientWithAuthorizer creates a registry client with the provided authorizer
func NewClientWithAuthorizer(url string, authorizer types.Authorizer, insecure bool, cliOpts func(cli *http.Client), interceptors ...interceptor.Interceptor) Client {
...
func (c *client) BlobExist(repository, digest string) (bool, int64, string, error) {
	...
	sdigest := resp.Header.Get("Docker-Content-Digest")
	length, _ := strconv.Atoi(resp.Header.Get("Content-Length"))

	defer resp.Body.Close()
	return true, int64(length), sdigest, nil
}
...
func (c *client) Copy(srcRepo, srcRef, dstRepo, dstRef string, override bool) error {
	...
	for _, descriptor := range manifest.References() {
		...
		// common layer
		default:
			exist, _, _, err := c.BlobExist(dstRepo, digest)
			if err != nil {
				return err
			}
